...
(function(vue) {
      ...
      getCurrentCity() {
        ...
        if (settings) {
          try {
            ...
            formatAppLog("error", "at pages/index/index.vue:156", "解析设置信息失败", e2);
          }
        }
    ...
    return vue.openBlock(), vue.createElementBlock("view", { class: "container" }, [
      ...
      vue.createElementVNode("view", { class: "header" }, [
        vue.createElementVNode(
          "text",
          { style: { "font-size": "40rpx", "font-weight": "bold", "display": "none" } },
    ...
    methods: {
      ...
      initCalendar() {
        ...
        for (let i2 = firstDay - 1; i2 >= 0; i2--) {
          this.prevMonthDays.push(prevMonthDays - i2);
        }
        const totalDays = firstDay + this.daysInMonth;
        const nextMonthDaysCount = 7 - (totalDays % 7 || 7);
        this.nextMonthDays = [];
        for (let i2 = 1; i2 <= nextMonthDaysCount; i2++) {
          this.nextMonthDays.push(i2);
        }
      ...
      // 年月选择器变化事件处理
      onYearMonthChange(e2) {
        const dateStr = e2.detail.value;
  ...
  const pages = [
    {
      ...
      style: {
        navigationBarTitleText: "AI日程管理小助理",
    ...
    {
      path: "pages/add-schedule/add-schedule",
      style: {
    ...
    {
      path: "pages/schedule-detail/schedule-detail",
      style: {
  ...
  const S = true, b = "app", T = I(define_process_env_UNI_SECURE_NETWORK_CONFIG_default), A = b, P = I('{"address":["127.0.0.1","192.168.31.221"],"servePort":7000,"debugPort":9000,"initialLaunchType":"local","skipFiles":["<node_internals>/**","D:/software/HBuilderX/plugins/unicloud/**/*.js"]}'), C = I('[{"provider":"aliyun","spaceName":"schedule","spaceId":"mp-ed020ef2-17df-422a-b8aa-20731df4346f","clientSecret":"hZ8RHCLUFg4dqpt17hHwSw==","endpoint":"https://api.next.bspapp.com"}]') || [];
  let O = "";
  try {
    O = "__UNI__A000033";
  } catch (e2) {
...
详细信息：${s4}`), r3(i3);
    ...
    methods: {
      ...
      recognizeText() {
        ...
        nr.callFunction({
          ...
          uni.hideLoading();
          formatAppLog("log", "at pages/add-schedule/add-schedule.vue:254", "云函数返回结果:", res.result);
          const result = res.result;
          if (result.code === 0 && result.data) {
            const scheduleData = result.data;
            formatAppLog("log", "at pages/add-schedule/add-schedule.vue:260", "提取的日程数据:", scheduleData);
              ...
              if (typeof scheduleData.date === "string") {
                if (scheduleData.date.includes("明天")) {
                  ...
                  try {
                    ...
                    formatAppLog("error", "at pages/add-schedule/add-schedule.vue:302", "日期解析错误:", e2);
                  }
                }
              }
            }
            ...
            formatAppLog("log", "at pages/add-schedule/add-schedule.vue:339", "处理后的日程数据:", recognizedInfo);
            this.schedule = {
              ...this.schedule,
              // 保留原有值
              ...recognizedInfo
            ...
            formatAppLog("error", "at pages/add-schedule/add-schedule.vue:354", "识别失败:", result);
            uni.showToast({
          ...
          uni.hideLoading();
          formatAppLog("error", "at pages/add-schedule/add-schedule.vue:364", "AI识别请求错误:", err);
          uni.showToast({
            ...
            uni.getFileSystemManager().readFile({
              ...
              success: (readRes) => {
                ...
                nr.callFunction({
                  ...
                  uni.hideLoading();
                  formatAppLog("log", "at pages/add-schedule/add-schedule.vue:410", "云函数返回结果:", res2.result);
              ...
              fail: (err) => {
                uni.hideLoading();
                formatAppLog("error", "at pages/add-schedule/add-schedule.vue:433", "读取图片失败:", err);
                uni.showToast({
                  title: "读取图片失败",
                  icon: "none"
                });
      ...
      // 切换天气分析开关
      toggleWeatherAnalysis(e2) {
        this.schedule.weatherAnalysis = e2.detail.value;
      },
      // 日期选择器变化处理
      onDateChange(e2) {
        this.schedule.date = e2.detail.value;
      },
      // 开始时间选择器变化处理
      onStartTimeChange(e2) {
        this.schedule.startTime = e2.detail.value;
      },
      // 结束时间选择器变化处理
      onEndTimeChange(e2) {
        this.schedule.endTime = e2.detail.value;
      },
      ...
      formatTimeString(timeStr) {
        ...
        try {
          ...
          formatAppLog("error", "at pages/add-schedule/add-schedule.vue:536", "时间格式解析错误:", e2);
        }
        return timeStr;
      }
    }
  ...
  function _sfc_render$1(_ctx, _cache, $props, $setup, $data, $options) {
    return vue.openBlock(), vue.createElementBlock("view", { class: "container" }, [
      ...
      $options.schedule ? (vue.openBlock(), vue.createElementBlock("view", {
        ...
        $options.schedule.weatherAnalysis && $data.weatherInfo && false ? (vue.openBlock(), vue.createElementBlock("view", {
          key: 2,
          class: "detail-card"
        }, [
  ...
  const _sfc_main$1 = {
    ...
    methods: {
      ...
      // 切换深色模式
      toggleDarkMode(e2) {
        this.settings.darkMode = e2.detail.value;
        this.saveSettings();
        uni.showToast({
      ...
      // 切换通知
      toggleNotification(e2) {
        this.settings.enableNotification = e2.detail.value;
        this.saveSettings();
      },
      // 切换天气分析
      toggleWeather(e2) {
        this.settings.enableWeather = e2.detail.value;
        this.saveSettings();
      },
      // 切换定位服务
      toggleLocation(e2) {
        this.settings.enableLocation = e2.detail.value;
        this.saveSettings();
      ...
      // 提醒时间变化处理
      onReminderChange(e2) {
        this.settings.reminderTime = parseInt(e2.detail.value);
        this.saveSettings();
      },
      // 导出数据
      exportData() {
  ...
  const store = createStore({
    ...
    getters: {
      ...
      todaySchedules: (state) => {
        ...
        return state.schedules.filter((schedule) => {
          ...
          return /* @__PURE__ */ new Date(`${a2.date}T${a2.startTime}`) - /* @__PURE__ */ new Date(`${b2.date}T${b2.startTime}`);
        });
      },
      // 获取本周日程
      weekSchedules: (state) => {
    ...
    mutations: {
      ...
      updateSchedule(state, updatedSchedule) {
        const index = state.schedules.findIndex((s2) => s2.id === updatedSchedule.id);
        if (index !== -1) {
      ...
      deleteSchedule(state, scheduleId) {
        state.schedules = state.schedules.filter((s2) => s2.id !== scheduleId);
        uni.setStorageSync("schedules", state.schedules);
      },
      // 设置当前日期
